...
class PhpDumper extends Dumper
{
    ...
    /**
     * Generates Services local temp variables.
     *
     * @param string $cId
...
EOF;
     ...
     * $return
     */
    {$visibility} function get{$this->camelize($id)}Services($lazyInitialization)
    {
...
EOF;
    ...
    private function addMethodMap()
    {
        ...
        ksort($definitions);
        foreach ($definitions as $id => $definition) {
            $code .= '            '.var_export($id, true).' => '.var_export('get'.$this->camelize($id).'Services', true).",\n";
        }
...
EOF;
    ...
    private function camelize($id)
    {
        ...
        if (!preg_match('/^[a-zA-Z0-9_\x7f-\xff]+$/', $name)) {
            throw new InvalidArgumentException(sprintf('Services id "%s" cannot be converted to a valid PHP method name.', $id));
        }
